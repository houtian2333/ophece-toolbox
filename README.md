...
系统清理中的部分操作（DISM 组件清理、USN 日志、系统还原点、关闭休眠、虚拟内存转移、事件日志）**需要管理员权限**。程序自动检测当前是否已提权：

- 未提权时，「系统清理」页顶部显示 **红色横幅**，并禁用需要管理员权限的勾选框
- 点击「以管理员身份重启」→ 触发 UAC → 以管理员身份重启（重启后所有系统操作均可用）
- 若在未提权状态下直接点击「执行系统清理」且勾选了需要管理员的任务，会弹窗提示并可一键提权重启

> 检测依据：Electron 主进程 `net session` 探测进程令牌；后端 `/api/admin/status`（`IsUserAnAdmin`）兜底。
...
| `GET` | `/api/admin/status` | 当前进程是否已提权 → `{"admin":true\|false}` |
...
这些操作需要**管理员权限**。右键 EXE →「以管理员身份运行」，或用程序内自带的「以管理员身份重启」按钮（系统清理标签页顶部横幅）。

**Q: 系统清理页的「以管理员身份重启」按钮在哪里？**
启动后未提权时，系统清理标签页顶部会自动显示红色提示横幅，内含该按钮。如果已经以管理员身份运行，横幅隐藏，工具栏显示 🛡 管理员 徽标。

**Q: 为什么有些系统清理选项是灰色的？**
DISM、USN 日志、系统还原点、关闭休眠、虚拟内存转移、事件日志这 6 项需要管理员权限。未提权时自动禁用，提权后自动可用。
